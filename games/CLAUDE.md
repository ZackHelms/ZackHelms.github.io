...
behind the ⚙: cel/toon (default), neon, 3D MODELS (real low-poly meshes that
bank into their turns) and 3D ANIMLIGHT (the same meshes flown 3x/2x in a
vivid palette, every hull running a rig of animated lights). **There is no menu screen**:
