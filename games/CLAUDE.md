...
slots, each erased by holding the bin beneath it. **Four graphics styles**
behind the ⚙: 3D ANIMLIGHT (**the default** — meshes flown 3x/2x in a vivid
palette, every hull running a rig of animated lights), 3D MODELS (the same
meshes, worn metal, no lights), cel/toon, and neon. The shield and the
...
true per-vertex perspective; **ANIMLIGHT** adds a vivid palette, 3x/2x hulls on
the field only, and a rig of animated lights on every hull. The choice persists per browser. Drones/shooters/spinners/tankers, P/S/G powerups (in-run
