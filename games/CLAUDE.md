...
| Editing a large single-file game | These are 600-4000-line single files, so the standard edit is a **span replacement between two anchors** (a python/`sed` block swap, or a large Edit). That silently deletes anything that happened to sit *between* the anchors: on 2026-08-25 a `drawPowerup` defined between `SHIELD_SWEEP` and `drawShip` was swallowed twice by two different rewrites of the code around it, each time surfacing only as a `ReferenceError` from inside `draw()` on the next test run. Before replacing a span, **list what is inside it** (`grep -n '^function\\|^const' <file> | awk` over the line range, or just read the tail of the region), and after any block rewrite parse-check the file (`node .claude/scripts/check-inline-js.cjs <page>`, ~100 ms, line numbers in HTML coordinates) before the smoke gate — never trust the diff to look right. Prefer anchoring on the *start and end of one function* over "from this declaration to that unrelated one" — `.claude/scripts/replace-fn.py` does exactly that by brace-counting, which is safer than any anchor pair you pick by hand. **The sharpest version of the trap is an end anchor that occurs INSIDE the region you meant to keep**: on 2026-09-20 a rewrite of interlock's `geometry()` anchored its end on `function fit(){`, and `fit` sat *inside* the viewport block that had been added just above it — so `viewBox()`, the canvas binding and the reflow wiring were all swallowed, and the page died with `viewBox is not defined`. An anchor pair is only safe if you have looked at what is between them **in the current file**, not in the version you remember. One more trap, hit **twice in one session** (2026-08-29): these files mix **literal Unicode** (`→`, `—`) with **escaped** forms (`\u2192`, `\u2014`), often on adjacent lines, so a text match copied out of your own head fails with a zero-count assertion on a line that looks identical in the terminal. Match on an ASCII-only prefix, `grep` the real bytes first (`cat -A`), or replace by **line range** located with a marker search — and always assert the match count rather than letting a silent no-op through. **That trap has a second stage** (2026-09-18): writing `\uXXXX` inside a shell heredoc can land the *literal character* in the file, and then "fixing" it by substituting ASCII breaks the file outright if the replacement is a quote — `\u2019` arrived as `’`, a tidy-up swapped it for `'` inside a single-quoted JS string, and the game stopped parsing. Escape the apostrophe (`\\'`) or pick the other quote, and **parse-check after any character-level substitution**, not just after a span replacement |
...
| Whole songs in WebAudio | Beyond a loop: five ~5-minute arrangements of fifteen independent stems each, synthesized live from pattern-string tables with **no audio asset at all** (music-mixer). The reusable parts are the data model (patterns + an arrangement, never a note list), a **cent**-based scale table so a song can be genuinely non-12-TET where its genre is, five parameterised voice factories rather than per-instrument functions, the Karplus-Strong workaround (a `DelayNode` in a feedback loop cannot go below one render quantum — ~375 Hz at 48 kHz — so bake the string in plain JS and pitch it with `playbackRate`), and the scheduling rule: **schedule every track every step and gate with a per-track `GainNode`**, because gating at schedule time means a pad pressed 50 ms before a beat misses that beat, which is exactly when a player reaches for a drum. Also non-optional: a limiter before the master (fifteen stems is a loud sum) and a per-song level trim, without which the song picker doubles as a volume control (measured 2.6x spread). Full derivation: `.claude/notes/20260917-webaudio-song-engine.md` |
...
| Rewarding skill (idle/incremental) | Whether skilled play actually pays is **measurable, and usually worse than it feels**: fire-clicker's optimal and naive personas arrived within **6% of each other** after thirteen simulated hours, which is the shape of a game where nothing the player does matters and is invisible from inside a single playthrough. Two personas measured against each other are a better instrument than either alone (`.claude/tests/eval-<slug>.cjs`). The fix that generalises: give the active player a bonus **the automation structurally cannot earn** — not a rule that says "no bonus for idlers", but two numbers that cannot both be satisfied. Fire-clicker's ROARING FIRE pays a camp-wide speed bonus above 75% of the fuel bank while the auto-stoker only tops up below 35%, so an auto-tended fire never crosses the line (measured: 100% vs 0.4% of the run). Then **assert the gap** — a balance property nobody asserts regresses silently. But assert it on **what the game is about**, not on a convenient number: fire-clicker measured the gap at POP 15/20 until a cost-curve flattening made recruits cheap for *everyone*, at which point the gap read 1.33x while optimal play still reached every stage twice as fast — and the naive persona reached POP 10 **first**, because the optimal opening buys throughput instead. A proxy that ranks naive play ahead of optimal play is measuring the proxy. When a balance assertion goes red after a curve change, **check whether the metric still discriminates before touching the game** (the first response here was to raise the skill lever's ceiling; it moved 1.33x to 1.40x and proved the lever was not the problem). Related: a skill lever still has to scale with the economy it is a lever on — fire-clicker's BELLOWS ceiling grows with the stage — or effort quietly stops mattering exactly as the numbers get big. Method: `.claude/notes/20260828-pacing-a-real-time-game-in-wall-clock-hours.md` |
...
| Multi-stage progression | A game with more than two civic/tech tiers must drive them from **one table**, not a chain of booleans. Fire-clicker gated everything on `S.up.village` — fine for two rungs, and the thing a binary cannot express is a third. The failure it invites is a settlement that goes modern in three painters and stays medieval in a fourth, because "is it a village?" gets re-answered independently at every site. One `STAGES[]` row owns everything that changes with scale (house cap, wall material, what the HEARTH is, ground surface, path style, lamp count), `stageIx()` walks the whole table rather than stopping at the first miss (so a save that skipped a rung still reports the truth), and every painter switches on a field rather than testing an id. The test that it worked: **adding a sixth rung should be one row plus one `case` per painter** — if it is more, something has drifted back out of the table. Two things scale together or the illusion breaks: caps GROW while building width SHRINKS (derive width from the column count, so a row always fits), which is what makes a denser grid read as denser rather than just taller. And keep the one prop every rung shares — fire-clicker's flame never changes, only its housing, which is what keeps the lighting anchor, the tap target and the bank ring pinned to one point across all five rungs while the campfire becomes a stove, a furnace, a steam plant and a reactor. Reference: `games/fire-clicker/` (`STAGES`, `stageIx()`, `drawFirePit`) |
...
| Hand-authored content data | A game whose content is a big hand-written data table — `HOLES[]`, `MISHAPS[]`, `STAGES[]`, `SIGNALS[]`, music-mixer's ~400 pattern strings — needs a **pure-node gate over that data**, separate from any browser suite. The reason is the failure shape: a mistyped table entry usually **does not throw**. It silently drops a note, shifts a pattern by a step, points at a variant that does not exist, or leaves a control that lights up and does nothing — none of which a diff shows and most of which are invisible until play happens to reach that entry. `eval` the data literal straight out of the page (brace-count the declaration out of the HTML; no browser, no deps, ~100 ms) and assert two layers: the **format contract** (every field present, every enum legal, every cross-reference resolving, every derived length a whole multiple of its unit) and the **design rules** the CD actually asked for (durations in band, nothing resting more than N% of its run, every resolvable value inside a sane range — that last one is how an octave typo gets caught before it ships as a subsonic lead). Music-mixer's found **39 real defects on its first run**, including four patterns one character short of a bar and two the arrangement referenced that no track had. Also refuse a **computed** entry (`'…'.replace()` to dodge a miscount): it evals into a pass and is unreadable in the file. Keep it in `.claude/tests/drive-<slug>.cjs` so `gates.sh` runs it by default — an `eval-` name puts it in the opt-in pacing bucket, which is not what a data gate is |
| Many-finger input | **Use Touch events, not Pointer events, and rebuild the held set from `e.touches` every time.** Safari DERIVES pointer events from touch events, so `preventDefault()` on `pointerdown` never reaches its gesture recognizer — only a **non-passive** `touchstart`/`touchmove` listener does. Leave it unprevented and the recognizer eventually decides a many-finger touch might be a system gesture and fires `pointercancel` **for the fingers already down**: music-mixer took five fingers happily and the sixth wiped all five (CD report, 2026-09-17). The second half of that bug was `addEventListener('blur', releaseEverything)` — Safari can blur the window for an instant while it makes that decision, so a handler that clears touch state on blur takes the grip with it. **Fingers are released by `touchend`/`touchcancel`, which are reliable; blur releases only keys and the mouse.** The architectural half matters most: an incrementally-maintained `Map` of pointerId keyed state **never recovers** from one missed or extra event, whereas `e.touches` is the authoritative list of every finger on the glass on *every* touch event, including `touchend` and `touchcancel` (which carry the fingers that REMAIN) — so rebuilding from it is self-healing, and one cancelled finger costs exactly one finger. Also put `touch-action:none` on whichever element the listeners are bound to, not just the visual one, and preventDefault `gesturestart`/`gesturechange`/`gestureend`. Every other multi-touch game here (air-hockey's 2P mallets, neon-clash's simultaneous trays, pinball's two-thumb flippers, phasic, ember-depths) was already on touch events — music-mixer was the only one that reached for Pointer Events, and the only one with a blur handler. Assert it as a **ladder**: add fingers one at a time and require the held count to climb and never fall (`.claude/tests/drive-music-mixer-runtime.cjs` § 5b), plus a single-finger cancel and a blur that both leave the rest of the grip alone. Full trace, the edges-vs-levels corollary for any latching input, and the CDP recipe for driving eight real fingers headlessly: `.claude/notes/20260917-many-finger-touch-ios.md`. **iPadOS caveat no page can fix:** four- and five-finger swipes and the five-finger pinch are system gestures above the browser, so a grip that large is unreliable on iPad until the CD turns multitasking gestures off |
| Cached hit-test geometry | **Anything that changes the layout must re-measure, not just `resize` and `orientationchange`.** A game that caches `getBoundingClientRect()` for its hit test (music-mixer's contact patch, block-fit's drop targets, any drag-to-a-slot puzzle) is reading where things *were*. Resize and orientation are the obvious triggers and every game already handles them; the one that bites is **your own chrome**. Music-mixer hides the tempo strip when no song is selected, which moves and resizes all fifteen pads — with no re-layout on that switch, every contact-patch check came up exactly **one pad short** (the eight-finger grip held 12 of 15) while the pads still lit normally under a finger. That is the failure mode worth designing against: a stale rect does not throw and does not look broken, it just quietly shrinks every target. **The severe case is a hidden element**: `resize` still fires and still runs your layout while a panel is `display:none`, and a hidden element measures as **zero** — so rotating the phone while music-mixer's key/mode screen was up left every pad rect an empty box, and coming back gave a grid that answered nothing at all. Re-measure on the way *back in*, not only on the way out. Put the `layout()` call in the function that toggles the chrome, not in the caller, so a new call site cannot forget it — and gate it by asserting the *geometry-derived* behaviour (a seam press holds two pads) **after** a mode switch, not only at boot. Make that assertion cross a **resize while hidden**, or it will not bite: the first version of music-mixer's check merely left the panel and passed happily with the re-measure deleted, because the panel's own show/hide did not change the grid's size. A check you cannot make fail is not evidence |
| Tuning a heuristic | **Tune on one set, report the number from another, and design the UI around the failures that remain.** A detector, generator, solver or difficulty curve tuned by hand against the cases you happened to try will fit those cases and nothing else. Build a synthetic set with KNOWN answers, grid-search the weights against it, then score a *separate* set you never tuned on and quote that number — music-mixer's tempo detector read 15/15 on its tuning set and 76% held out, and only the second figure is true. Two things follow. First, put the search in a scratch harness, not the game: the weights ship, the search does not. Second, and more important, **read the residual failures and answer them in the interface**. Tempo detection fails as an octave error (double or half, almost never 7% off), so the edit bar carries one-tap /2 and x2 rather than only a number field — and those refuse out-of-range instead of clamping, because clamping means x2 then /2 does not return where it started. A heuristic you cannot make perfect is not a reason to hide its output; it is a reason to make correcting it cost one tap |
| Adding a mode to a single-mode game | **Every place that assumed the old single mode is now a bug, and none of them throw.** Music-mixer went from one kind of song to three (a built-in, a blank recording kit, a recorded take) and four defects fell out, all the same shape: an `if (!S) return;` guard that silently muted recorded songs while every pad still lit; a `voice(S.tracks[i].v)` reading the voice off the only kind of song that has tracks; an `ensureAudio()` that started "the" transport and so recursed with the new mode's `takePlay()`; and an `applyAudioSong()` that opened all the track gains for the new mode and thereby un-muted the outgoing mode's still-ringing notes. **Grep for the old mode's own variable before writing the new one** (`S`, `level`, `board` — whatever the singular used to be) and make every hit either mode-aware or explicitly mode-guarded. Two traps worth naming: a function that *starts* a subsystem must be split from one that merely *wakes* it, or the two call each other; and anything that opens gates, unpauses or resets on a mode switch has to consider what the mode it is LEAVING still has in flight (WebAudio notes already scheduled, a gong with a six-second tail). Gate it by asserting each mode's contract separately **after switching into it**, never only at boot |
| A check that can skip | **A gate branch that can quietly do nothing is worse than no gate, because it reports GREEN.** Music-mixer's export section ran straight after a page-reload check that leaves nothing selected, so the value it exported was `null`, `JSON.parse('null')` returned `null` without throwing, an `if (J) { …twenty assertions… }` skipped every one of them, and the suite passed having tested nothing. Write the absence as a **failure**, not a skip: `if (!thing) fail(…)` then `else { …assertions… }`. Same rule for a loop that never iterates, a `for (const x of list)` over an empty list, and a `try/catch` that swallows a setup error. The cheap way to catch it is the one this repo uses everywhere — **break the thing on purpose and confirm the gate goes red**; a check you have never seen fail is not evidence. **Do that through `.claude/scripts/negtest.sh save|restore`, not hand-rolled `cp` backups.** The script exists for the one outcome that actually hurts — a break you failed to restore — and `restore` cmp-verifies, so a `cp` that succeeded against the wrong path is caught on the spot instead of shipping. Skipping it cost an interlock session most of an evening on 2026-09-20: four breaks restored by hand, one `cp` ran from a drifted working directory (the identical failure the script's header records from 2026-08-23), and a line that overwrote the night sky with daytime blue stayed live through several gate runs. The sweep that was supposed to catch it checked three of the four markers and missed the fourth, which is the general lesson — **a hand-written sabotage sweep must enumerate every break you applied, not the ones you remember**, and that is exactly the bookkeeping the script does for you. `negtest.sh scan` now also fails on a snapshot that was saved and never restored, so the correct workflow is self-enforcing rather than a matter of memory; the `@negtest` marker only ever protected a break someone remembered to mark. Related hazard in the same suite: a visibility-gated `page.click()` on an element an earlier failed check left closed **times out** and kills the run on a Playwright error instead of reporting the real problem, so dispatch those clicks directly. **Two more shapes of the same failure, both from 2026-09-19.** (1) *A check that quotes a magic number out of prose goes vacuous when the prose changes.* The export gate asserted the readme contained `285` — the song-duration floor; when the CD widened the window to 120 s, that check would have gone on passing against a sentence that no longer existed, testing nothing. Assert the *current* sentence, and when you change a rule, grep the gates for its old number before you change the prose. (2) *A measurement check can be toothless because of how it measures.* Breaking the fix on purpose caught 5 of 7 cases; the two misses were the check's fault, not the code's — one anchored its search on the loudest sample (which for a slow-attack voice sits inside the very artifact it is hunting) and one measured while the previous case was still ringing. **Negative-test every row, not the check as a whole**: a suite that goes red proves *something* fired, not that each case can. **Four more shapes, all from one 2026-09-20 rendering pass** (`.claude/notes/20260920-measuring-a-rendering-change.md` has the trace) — every one of them produced a confident number that was measuring the wrong thing. (1) *Two page loads are two different boards.* A shadow check compared builds with shadow mapping compiled in and out, read "20% of the frame changed", and was really measuring two different procedurally-generated puzzles; toggled at runtime on ONE board the honest figure was 0.4%. Any game here with a seeded or random layout has this trap: **toggle the feature in the live page, never diff two loads**. (2) *Comparing two variants is not a causal test.* "Is the glass transmitting?" was first asked by comparing interior detail across materials, and could not discriminate at all — opaque pine's wood grain scored HIGHER than two of the three glasses. Expose a test-only toggle and measure the same scene with the feature on and off. (3) *Aim where the error is largest, and more than once.* A tap-accuracy row aimed at a target near the screen's centre line, where the coordinate-space error it was hunting is zero by construction; it passed with the bug deliberately reinstated. It now scans for the reachable point furthest off centre and takes three turns, because one lucky board was carrying it. (4) *Wait in rendered frames, never milliseconds.* A suite that passed alone went red inside `gates.sh`, where several Chromium suites run back to back; nothing about the page had changed, the fixed `waitForTimeout(400)` had simply become zero frames under load. On a WebGL page in this container a frame can cost 300-700 ms — wait on `requestAnimationFrame` counts, or better on the property itself |
| A window-level key handler and a text field | **The moment a game grows any `<input>`, its global `keydown` is stealing characters.** Music-mixer binds fifteen letters and digits to pads; the recorder added a song-title box and a BPM field, and the CD reported that typing a name dropped most of it — `preventDefault()` on the pad keys ate every letter that happened to be one, so "SAD FROG 12" arrived as " O ". It reads as a flaky text field rather than as a key binding, because the letters that survive are the ones nobody thought about. Ask `document.activeElement` for INPUT / TEXTAREA / SELECT / `isContentEditable` **first**, before the key even resolves to an action. Guard `keydown` only, never `keyup` — a key pressed before the field took focus still has to be able to come back up, or it sticks down forever. Gate it by typing a string made of bound keys and asserting both halves: the field got the text *and* nothing was held. Same rule for any element that scrolls, and for a modal that should be answering Escape |
| Derive it, or it goes stale | **A value computed once at creation and then stored is a lie as soon as the player can change its inputs.** Music-mixer stored a take's length in bars at the tempo detected when it was recorded; correcting the BPM in the edit bar halves every note's position but left `baseBars` where it was, so the loop either grew a silent bar or wrapped notes on top of each other. The fix is not to recompute it in more places — it is to stop storing it: keep only what was actually observed (here, exact tap times in seconds) and derive everything expressed in the *current* interpretation (steps per bar, bar count, duration) on every pass of the function that prepares playback. The test for whether a field belongs in storage is whether it could ever disagree with the others; if it can, it will, and the disagreement will be silent. Storage then also gets smaller and its migration rules get simpler, since a field you never read cannot be corrupt |
| Web Audio automation runs in TIME order | **Not in the order you called the methods.** Every `setValueAtTime` / `linearRampToValueAtTime` / `exponentialRampToValueAtTime` on an `AudioParam` goes into one timeline sorted by its timestamp, so a ramp you scheduled *last* can take effect *first*, and a ramp whose end time lands after a later event's runs after it. Music-mixer's sustaining envelope scheduled attack, decay-to-sustain at `t+atk+dec`, then hold and release at `t+dur` — correct for a long note, and for a note shorter than `atk+dec` it put the decay ramp after the release: the note faded out on the beat and then **swelled back up to the sustain level** with no event after it to stop it, a ghost about half a second late (2026-09-19, CD heard it as "sounds off rhythm"). Clamp every envelope segment to the note's own length and carry the value forward (`p * sus^((dEnd-atk)/dec)`) rather than assuming the segment completed. The general rule: **an envelope's segments must be monotonically increasing in time for every duration the voice can be handed**, and the durations come from data, so they will eventually be shorter than you designed for. It is invisible in a mix and obvious in the envelope, so gate it by firing ONE note into a dry `AnalyserNode` (muted into the graph so it is pulled) and asserting the level never returns after it has fallen — a 15-line check that caught the same defect in seven voices across five songs. Getting that check to have teeth took three tries and all three traps are worth knowing: do not anchor on the peak (for a slow-attack voice the ghost is LOUDER than the note, so the maximum sits inside the artifact — count humps instead), wait out the previous voice (oscillators outlive their envelope and a gong runs 6.8 s, so the next row reads its tail), and do not fire each voice at its shortest note in the data (13% short is too small a margin to detect — fire well inside attack+decay, since what is guarded is the envelope function, not the song). Full derivation, with sparklines: `.claude/notes/20260917-webaudio-song-engine.md` § Envelopes |
...
**Use `.claude/scripts/stamp-badge.sh <page ...>`, not a hand-written `sed`.**
The obvious one-liner is `sed -i "s/build 2026-09-18 [0-9:]* UTC/$STAMP/"` —
and it hardcodes today's date, so it **silently no-ops the moment the date
rolls over in UTC** or the badge holds a placeholder of a different shape. The
page then ships with a stale badge and the report quotes a timestamp the file
does not contain, which is exactly the failure the badge exists to catch. The
script rewrites the badge div's whole text regardless of what is in it and
exits non-zero if a file did not actually change (2026-09-18: five hand-sed
restamps in one session, each one a date-boundary away from doing nothing).
...
going further pays. **MISHAPS** make the civic buildings insurance rather than
multipliers: injuries, blazes and a thief who sneaks between the buildings with
a visibly growing sack, all escalating off the stage ladder, all closed faster
by tapping them and by the rank of building that answers them — and the thief
gets clean away unless you spot him and have the law to back the catch.
The long road from here is a **Kardashev ladder** to a Type III civilization,
...
### INTERLOCK (`interlock/index.html`, generated from `interlock/src/`)
A 3-D disassembly puzzle, supplied finished by the CD (2026-09-19) and the only
game here that **bundles a third-party renderer** — Three.js r169, pinned and
MIT, a documented carve-out from the no-external-libraries rule rather than a
precedent. A solid cube is subdivided into connected polycube pieces; drag to
rotate, tap a piece to try all six straight exits. A blocked piece jiggles, a
clear one slides out; clear the board and the next puzzle is one piece bigger.
There are no authored levels, so the generator IS the content and an unsolvable
board is silent — `.claude/tests/drive-interlock.cjs` proves 192 seeded boards
across every size 5-20 can each be taken fully apart.

**`index.html` is GENERATED** by `build.mjs` from `src/`; edit the source. The
bundler exists because the repo wants one self-contained file and because
`smoke-mobile.cjs` loads pages over `file://`, where an ES-module import is a
CORS error — so a multi-file page would report RED forever. Three.js stays at
module top level with its export list rewritten into one namespace object while
the game's modules go inside an IIFE, because three's ~450 minified top-level
names (`t`, `e`, `n`, `s`…) collide with the game's own.

Deliberately **not** the neon aesthetic. Lit by the sun and the moon and
nothing else, with ambient baked from the same sky shader the player is looking
at; one shadow-caster at a time, plus per-vertex voxel AO recomputed on every
removal. Its edge lines are **opaque** on purpose — three renders only the
