...
| Music Mixer (`games/music-mixer/`) | `.claude/music-mixer.md` | Any work on the 15-pad stem mixer — the **two pad modes** (hold, and the tap-to-lock toggle whose locks deliberately outlive a switch back to hold, so `recompute()` handles input as **edges** and `recompute(noLatch)` keeps a programmatic release from clearing a lock), the **contact-patch** hit test that makes one fingertip on a seam hold two pads and eight fingers hold fifteen (and why fingers go through **touch** events, never pointer events), the pad layout contract (hue climbs each column in turn, red bottom-left to magenta top-right, percussion the right column with fixed slot roles across all five songs), the **notation contract** for ~400 pattern strings plus the arrangement, the five voice factories and the Karplus-Strong bake, the **cent**-based scales that make gamelan really slendro and raga really just-intonation, per-song level trims, the schedule-everything/gate-with-gain scheduler, and the rule that it renders in **DOM/CSS rather than Canvas 2D** |
