...
| Neon Clash (`games/neon-clash/`) | `.claude/neon-clash.md` | Any work on the real-time card skirmish — the rotated two-player tray, the unit/building/**spell** card-type contract every rule branches on, the deploy contract (drag **or** tap-to-arm, and the border rule that clamps a cross-line aim onto your own side), bunker/garrison rules, the fireball falloff, the siege lock, the base turret and its rim-measured envelope, unit stats, the AI ladder |
