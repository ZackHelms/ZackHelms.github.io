...
- `experiments/` — **not games.** Interactive dioramas probing web rendering/sensor tech
  ...
  **ARViewport** (`experiments/rain-on-glass/`, scenes × times × weather × beings +
  ...
  2026-08-15.
- `games/cyoa/` — AI Game Master tabletop RPG (CD commission 2026-09-25). Claude
  narrates over the **player's own API key** (the only page that calls an API at
  runtime); a deterministic engine owns the world. Built in the **Grimoire** style,
  with no top-left chrome and a 2x reload button by the CD's call. Protected. See
  `.claude/cyoa.md`
...
- `games/neon-clash/models/` — **3D source** for Neon Clash's pre-rendered `sprite`
  ...
  then `cd games/neon-clash/models && node build.mjs` (~45 s)
- `games/neon-clash/sprites/` — **generated, do NOT edit by hand.** Written by
  that build script, which also re-inlines the atlas manifest into the game
...
- **Protected games — NEVER add a permissive license to these seven:**
  `games/fire-clicker/`, `games/phasic/`, `games/mitochondria/`,
  `games/qntmchmst/`, `games/turret-builder/`, `games/interlock/`,
  `games/cyoa/`. The CD may
  ...
  which stays MIT on its own terms. `games/cyoa/` was protected on the CD's
  call before it was built (2026-09-25).
...
|---|---|---|
| CYOA (`games/cyoa/`) | `.claude/cyoa.md` | Any work on the AI Game Master game — the CD's standing decisions (bring-your-own key, Opus 5.5 default, conversational character creation, a three-act main quest in an open world, no top-left chrome, the 2x reload button), the seeded **world bible** and the rule that it travels inside every save, the **engine as the only door** (23 tools that validate before they mutate, an event log that replays to the exact state, turn atomicity on a deep copy), the **cached prompt prefix** that must stay byte-stable, each turn as a fresh conversation so no thinking block is ever replayed across turns, the raw-fetch SSE client and its per-model request shapes, the narrator's sentence queue, the woodcut plates keyed by place, the **cost ledger** (Claude priced exactly from usage and the served model, voice and pictures estimated, saved per game, a $ history on the Load screen, per-task model routing), the **never-innerHTML** rule that protects the key, and the Grimoire style |
| Interlock (`games/interlock/`) | `.claude/interlock.md` | Any work on the 3-D disassembly puzzle — the `src/` → `build.mjs` → generated `index.html` pipeline and the scope trick that makes the bundle possible, the polycube generator and its solvability gate, the standard chrome row painted in the game's own glass rather than the neon palette, the **lighting model** (sun and moon and nothing else, ambient baked from the sky shader itself, one shadow-caster at a time, voxel AO recomputed on every removal) and the **opaque edge lines** that are the only reason the other pieces are visible through the glass — three renders just the opaque list into the transmission target — plus the Three.js carve-out and its **proprietary-license** status (one of the seven protected games) |
...
| Star Surge (`games/star-surge/`) | `.claude/star-surge.md` | Any work on the shmup — the canvas title screen (a logo spelled in enemy hulls over a procedural 3D dogfight), the five graphics styles (cel, neon, the 3D-models mesh kit with painter-derived banking, 3D animlight — the default, those meshes in a vivid palette under animated per-hull light rigs — and SPRITESHEETS, that art pre-rendered to sprite frames with the title dogfight flattened into a recorded loop; the shield and powerups are skinned per style too, and EVERY style flies the field hulls oversized, 3x/2x/1.5x, with no hitbox or pickup reach moved), the station hub and routing, wave director, boss phases, powerups, bullet caps |
...
| Ember Depths (`games/ember-depths/`) | `.claude/ember-depths.md` | Any work on the roguelike — turn engine, floor generation, vision/light map, **stacking** relics (every effect site reads `relicCount()`, never `relics.includes()`), the board camera (`applyView`, the `camFree` free-look latch and the ⌖ button that is the only way back, `canPan()` rather than `zoom > MIN_ZOOM`), the gesture latch that keeps a pinch from firing a tap, the tap-the-relic-row detail panel (a read, never a turn), the settings panel, and the **meta layer** — three character slots under one `emberDepths.slots.v1` blob that `migrate()` never trusts field-by-field, the camp's gear/skills/supplies tabs, the rule that the kit is folded into `player` exactly once in `startRun()`, the **one-DAG skill tree** (two roots, `skillOpen()` needs EVERY input node, capstones under both sides) with points earned one per five depths on first completion, the **backpack** (using a supply is what spends it, not dying with it), traps and the `statuses` they leave — and the auto-path is **blind to them on purpose**, so do not "fix" it — plus the two tuned tables, the unbounded `CURVE` and the geometric `GEAR` ladder, both set against `.claude/tests/eval-ember-depths.cjs` (`--depth` for the curve, `--campaign` for the economy) and never guessed |
