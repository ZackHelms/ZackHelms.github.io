...
/* --------------------------- built-in personas --------------------------
   `lab` is a priority list of track ids retried in order every time cores
   are spent; repeat an id to buy several tiers. A SHORT list strands cores
   the same way grid-defense's short skill lists stranded points, so the
   report prints what it could not spend. */
const LAB_ALL = [
  'grid', 'chassis', 'grid', 'fire', 'chassis', 'grid', 'arc', 'servo',
  'blast', 'chassis', 'ice', 'grid', 'bulwark', 'optics', 'fire', 'chassis',
  'arc', 'grid', 'servo', 'blast', 'ice', 'bulwark', 'optics', 'fire',
  'arc', 'blast', 'ice', 'servo', 'optics', 'bulwark', 'chassis', 'bulwark',
];
const MIXED = { fire: 0.28, arc: 0.26, blast: 0.24, ice: 0.22 };

const PERSONAS = [
  { name: 'coherent', desc: 'mixed modules, shared between packed turrets, walls up, lab spread',
    mods: MIXED, gridSmart: true, walls: 2, wallMods: ['ice', 'fire'],
  ...
  { name: 'orphan-mods', desc: 'buys modules but drops them wherever — nothing touches a turret',
  ...
  { name: 'fire-only', desc: 'every module is FIRE — all elemental, nothing for a WARD',
    mods: { fire: 1 }, gridSmart: true, walls: 2, wallMods: ['fire'],
    lab: LAB_ALL, upgrade: 'value', place: 0.45 },

  { name: 'ice-only', desc: 'every module is ICE — slows everything, kills nothing fast',
    mods: { ice: 1 }, gridSmart: true, walls: 2, wallMods: ['ice'],
    lab: LAB_ALL, upgrade: 'value', place: 0.45 },

  { name: 'arc-only', desc: 'every module is ARC — the crowd module, measured strongest',
    mods: { arc: 1 }, gridSmart: true, walls: 2, wallMods: ['ice', 'fire'],
    lab: LAB_ALL, upgrade: 'value', place: 0.45 },

  { name: 'blast-only', desc: 'every module is BLAST — the only module that deals BOTH damage types',
    mods: { blast: 1 }, gridSmart: true, walls: 2, wallMods: ['ice', 'fire'],
    lab: LAB_ALL, upgrade: 'value', place: 0.45 },

  { name: 'no-walls', desc: 'a good grid that never buys a second of time on the road',
    mods: MIXED, gridSmart: true, walls: 0, wallMods: [],
    lab: LAB_ALL, upgrade: 'value', place: 0.45 },
...
const PLAYER = `(() => {
  ...
  function turretScore(strat, r, c) {
    ...
    // a turret with room around it can actually be wired up later
    if (!strat.scatter && !strat.noMods) {
      let free = 0;
  ...
  function modCount(t) { return MODS.concat(['armor','regen']).reduce((a, k) => a + t.mods[k], 0); }
  function modShare() {
    const tally = { fire:0, ice:0, arc:0, blast:0 };
    let n = 0;
...
(async () => {
  ...
  for (const persona of personas) {
    ...
    if (!AS_JSON) {
      ...
      console.log('   ' + line);
      console.log(`   board: ${res.turrets} turrets · ${res.modules} modules · ${res.walls} walls · ${res.links} live links` +
  ...
  const unshared = by('unshared'), fireOnly = by('fire-only'), iceOnly = by('ice-only');
  ...
  /* The design's one genuinely novel rule: a module feeds EVERY structure it
     touches. If refusing to share cost nothing, the rule would be decoration. */
  ...
  /* Armour vs resist: the SINGLE-TARGET one-note grids must meet the creep
     that answers them. */
  check('a FIRE-only grid stalls before a mixed one', depth(fireOnly) < depth(coherent),
    { fireOnly: depth(fireOnly), coherent: depth(coherent) });
  check('an ICE-only grid stalls before a mixed one', depth(iceOnly) < depth(coherent),
    { iceOnly: depth(iceOnly), coherent: depth(coherent) });

  /* The CROWD modules are a different story, and this is a measured fact
     pinned here rather than a wish. ARC puts 20% of the hit on five targets
     and BLAST deals both damage types, so both clear the campaign alone and
     outscore an evenly-mixed grid. That ordering follows from the spec's own
     percentages; it is asserted so that a future rebalance cannot invert it
     silently, in either direction. Full note: .claude/turret-builder.md
     § Module power — the measured ordering. */
  const arcOnly = by('arc-only'), blastOnly = by('blast-only');
  check('ARC alone clears the campaign — chain damage scales with the crowd',
    arcOnly.stalled === null, depth(arcOnly));
  check('BLAST alone clears the campaign — it is the only module dealing both types',
    blastOnly.stalled === null, depth(blastOnly));
  check('the crowd modules outrank the single-target ones, as the spec’s percentages imply',
    depth(arcOnly) > depth(fireOnly) && depth(blastOnly) > depth(iceOnly),
    { arc: depth(arcOnly), blast: depth(blastOnly), fire: depth(fireOnly), ice: depth(iceOnly) });
